...
#define P_CONFIG_REG_GAIN_SETTING FBM340_P_CONFIG_REG_GAIN_X4
...
int fbm340_calculation(struct fbm340_data *barom)
{
	...
	case hw_ver_b3:
		...
		X26 = ((X21 >= X22 ? ((0 - X24 - X25) >> 11) : ((X24 + X25) >> 11)) + cali->C7 + 166426) >> 2;

#define RATIO_FACTOR_PG16_TO_PG64 (85)
		UP = (((UP - 8388608) * RATIO_FACTOR_PG16_TO_PG64) >> 16) + UP;

		PP1	= ((UP - 8388608) - (X13 >> 2)) >> 2;
		PP2	= (X26 >> 10) * PP1;
		PP3	= ((X26 & (0x3FF)) * PP1) >> 10;
		PP4	= (PP2 + PP3) >> 8;

		if (PP4 > 0) {
			LCS	= ((DT2 * 5123) >> 20) + 1091;
			LCI	= ((DT2 * 6152) >> 12) + 262804;

			dC07 = cali->C7 - 262144;
			LC = (((dC07 * LCS) >> 10) - LCI) >> 8;
			X31 = (abs(LC) >> 11) * PP4;
			X32	= ((abs(LC) & (0x7FF)) * PP4) >> 11;
			...
			X31	= (abs(CF) >> 11) * PP4;
			X32	= ((abs(CF) & (0x7FF)) * PP4) >> 11;

			if (CF > 0) {
				RP = PP4 - ((X31 + X32) >> 1) + 100000;
			} else {
				RP = PP4 + ((X31 + X32) >> 1) + 100000;
			}
		} else {
