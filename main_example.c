...
int32_t main(void)
{
	...
	while (1)
	{
		...
		if (fbm340_update_rdy) {
			...
			/* This function converts pressure value into altitude in unit of millimeter(mm). */
			altitude = fbm340_get_altitude(real_p);
			/* This function converts pressure value into the depth under water level in unit of millimeter(mm). */
